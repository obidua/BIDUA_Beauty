    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black flex items-center justify-center px-4 py-16">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-4xl font-bold text-white mb-4">{t('userProfile.notLoggedIn.title')}</h1>
          <p className="text-gray-400 mb-8 text-lg">{t('userProfile.notLoggedIn.subtitle')}</p>
          <Link 
            to="/login" 
      ...
      <div className="container mx-auto max-w-4xl relative z-10">
        ...
        <div className="bg-gradient-to-br from-gray-800/50 to-black/50 backdrop-blur-sm border border-gray-700/50 rounded-3xl p-6 lg:p-8 mb-12">
          ...
          <div className="space-y-4 text-gray-300">
            <p className="flex items-center">
              ...
              <span className="font-medium text-white">{t('userProfile.email')}:</span> 
              <span className="ml-2">{user.email}</span>
            </p>
            <p className="flex items-center">
              <Phone className="w-5 h-5 mr-3 text-gray-400" />
              <span className="font-medium text-white">{t('userProfile.phone')}:</span> 
              <span className="ml-2">+91 98765 43210 (Static)</span>
            </p>
            <p className="flex items-center">
              <Briefcase className="w-5 h-5 mr-3 text-gray-400" />
              <span className="font-medium text-white">{t('userProfile.memberStatus')}:</span> 
              {user.isBrppMember ? (
                <span className="ml-2 bg-green-500/20 text-green-400 px-3 py-1 rounded-full text-sm font-bold">{t('userProfile.brppMember')}</span>
              ) : (
                <span className="ml-2 bg-gray-500/20 text-gray-400 px-3 py-1 rounded-full text-sm font-bold">{t('userProfile.standardUser')}</span>
              )}
            </p>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          ...
          >
            ...
            <h3 className="text-xl font-bold text-white">{t('userProfile.myWallet')}</h3>
            <p className="text-gray-300 text-sm">{t('userProfile.viewEarnings')}</p>
          </Link>
          <Link 
            to="/b2b-catalog" 
          ...
          >
            ...
            <h3 className="text-xl font-bold text-white">{t('userProfile.brppProgram')}</h3>
            <p className="text-gray-300 text-sm">{t('userProfile.joinBrpp')}</p>
          </Link>
        </div>
